...
static ZeroEngine::Application* s_Instance = nullptr;
...
namespace ZeroEngine {
	...
	void Application::Init()
	{
		...
		// Load images
		{
			...
			m_AppHeaderIcon = std::make_shared<ZeroEngine::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowMinimizeIcon, sizeof(g_WindowMinimizeIcon), w, h);
			m_IconMinimize = std::make_shared<ZeroEngine::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowMaximizeIcon, sizeof(g_WindowMaximizeIcon), w, h);
			m_IconMaximize = std::make_shared<ZeroEngine::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowRestoreIcon, sizeof(g_WindowRestoreIcon), w, h);
			m_IconRestore = std::make_shared<ZeroEngine::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowCloseIcon, sizeof(g_WindowCloseIcon), w, h);
			m_IconClose = std::make_shared<ZeroEngine::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
	...
	void Application::UI_DrawTitlebar(float& outTitlebarHeight)
	{
		...
		// Minimize Button
		ImGui::Spring();
		ZeroEngine::UI::ShiftCursorY(8.0f);
		{
	...
	void Application::Run()
	{
		...
		const float frameDuration = 1.0f / targetFPS; // Duration of each frame
		ImGui_ImplVulkanH_Window* wd = &g_MainWindowData;
		...
		while (!glfwWindowShouldClose(m_WindowHandle) && m_Running)
		{
			...
			float frameTime = GetTime() - m_LastFrameTime; // Calculate how long the frame took
			m_LastFrameTime = GetTime();
			if (frameTime < frameDuration)
			{
				std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>((frameDuration - frameTime) * 1000.0f)));
			}
